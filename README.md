...
- **Reading a long session** — `ctrl+s` searches the transcript as you type, marks every match, cycles them with `ctrl+n`/`ctrl+p`, and puts the scroll position back where it was when you close it. `alt+↑`/`alt+↓` jump by turn, `ctrl+t` collapses the history above the current exchange to one line per turn, `ctrl+f` folds long code blocks to a summary line, and `ctrl+y` copies the last one to the clipboard as raw text. Diffs are coloured by hunk (and by the basic ANSI pair on a 16-colour terminal, where a hex green would be approximated away), and tables truncate their widest column instead of wrapping into confetti.
