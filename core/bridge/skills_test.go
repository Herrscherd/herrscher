...
func TestHubInjectsSkillMenuAndExpandsOnMarker(t *testing.T) {
	...
	sink := &recordSink{}
	runOneTurn(context.Background(), sink, resp, nil, contracts.Event{T: "input", Text: "hi"}, nil, eng, affordances{})
	...
	runOneTurn(context.Background(), &recordSink{}, resp, nil, contracts.Event{T: "input", Text: "again"}, nil, eng, affordances{})
...
func TestHubSkipsInjectionWhenEngineNil(t *testing.T) {
	...
	runOneTurn(context.Background(), &recordSink{}, resp, nil, contracts.Event{T: "input", Text: "hi"}, nil, nil, affordances{})
	if resp.contexts[0] != "" {
