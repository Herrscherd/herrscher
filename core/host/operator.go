...
// forwardSessionCommands sends every session verb to the running daemon instead
// of running it here.
//
...
// the session back. The daemon owns the live sessions, so it is the one that
// must decide; this process only asks.
//
...
func forwardSessionCommands(reg *cli.Registry, instID string, forward seedCommandForwarder) {
	reg.Intercept([]string{"session"}, func(cmd contracts.Cmd, next cli.Runner) cli.Runner {
